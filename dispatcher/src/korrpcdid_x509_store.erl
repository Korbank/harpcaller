...
known(Certificate = #'OTPCertificate'{}) ->
  ...
  CertSubject = korrpcdid_x509:subject(Certificate),
...
-spec read_certs(file:filename()) ->
    {ok, [korrpcdid_x509:certificate()], change_time()} | {error, term()}.

read_certs(File) ->
  case korrpcdid_x509:read_cert_file(File) of
    {ok, Certs} ->
...
-spec store([korrpcdid_x509:certificate()]) ->
  cert_store().
...
-spec add(korrpcdid_x509:certificate(), cert_store()) ->
  cert_store().

add(Cert, CertStore) ->
  Subject = korrpcdid_x509:subject(Cert),
  dict:append(Subject, Cert, CertStore).

%% @doc Check if the store has this particular X.509 certificate.

-spec has(korrpcdid_x509:certificate(), list(), cert_store()) ->
  boolean().
