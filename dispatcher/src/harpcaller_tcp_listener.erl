  ...
  case resolve(Addr) of
    {ok, BindAddr} ->
      case listen(BindAddr, Port) of
        {ok, Socket} ->
          harpcaller_log:info("listening on socket",
          ...
          {ok, State, 0};
        {error, Reason} ->
          harpcaller_log:err("can't setup listening socket", [
            {action, setup},
            {error, {term, Reason}},
            {bind_address, format_address(BindAddr)}
          ]),
    ...
    {error, Reason} ->
      harpcaller_log:err("can't resolve bind address",
...
handle_call(rebind = _Request, _From, State) ->
  case rebind_state(State) of
    ...
    {ok, NewState = #state{bind_address = NewBindAddr}} ->
      harpcaller_log:info("rebound to a new address",
...
handle_call(shutdown = _Request, _From, State) ->
  harpcaller_log:info("shutting down listening socket"),
...
-spec resolve(harpcaller:address()) ->
...
-spec format_address(harpcaller:address(), inet:port_number()) ->
  binary().
