...
%%%   intervals.
%%%   After that the process sends collected data to {@link korrpcdid_hostdb}.
%%% @end
%%%---------------------------------------------------------------------------

-module(korrpcdid_hostdb_refresh).
...
init(_Args) ->
  ...
  % it's up to korrpcdid_hostdb to decide that hosts database needs immediate
  % refreshing (korrpcdid_hostdb can read host_db_refresh on its own)
...
-spec decode_host_entry(string() | binary()) ->
  korrpcdid_hostdb:host_entry() | ignore.
